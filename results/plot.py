...
def quick_plot_fec(df, target_size, fec_ratio, coding):
    if df is None:
    ...
    y = [quality, quality, 8]
    plt.plot(x, y, label=f"{coding} {fec_ratio*100:.1f}% FEC")
...
for nframes in [1]:
    ...
    if df_265 is not None:
        quick_plot_fec(df_265, size, 0.2, 'H.265')
        quick_plot_fec(df_265, size, 0.5, 'H.265')
    if df_264 is not None:
        quick_plot_fec(df_264, size, 0.2, 'H.264')
        quick_plot_fec(df_264, size, 0.5, 'H.264')
