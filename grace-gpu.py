...
import os, sys
os.environ['LD_LIBRARY_PATH'] = '/home/wangyihui/code/Grace/libs:' + os.environ.get('LD_LIBRARY_PATH', '')
...
def read_video_into_frames(video_path, frame_size = None, nframes=1000):
    ...
    Input:
        ...
        nframes: sample front number of frames
    Output:
...
def bpg_encode(img):
    # 将图像值限制在 [0, 1] 范围，并转换为 PIL 图像
    frame = (torch.clamp(img, min=0, max=1) * 255).byte()
    _, h, w = frame.shape
    frame = frame.permute(1, 2, 0).numpy()  # 转换为 (h, w, c) 格式
    pil_image = Image.fromarray(frame)

    # 使用 BytesIO 将图像保存为 JPEG 格式的字节流
    with io.BytesIO() as output:
        pil_image.save(output, format='JPEG')  # 可以尝试 'JPEG' 或其他格式
        bpg_stream = output.getvalue()
...
def bpg_decode(bpg_stream, h, w):
    # 将字节流转换为 BytesIO 对象
    bpg_bytes_io = io.BytesIO(bpg_stream)

    # 使用 Pillow 打开图像
    pil_image = Image.open(bpg_bytes_io)

    # 将图像转换为 NumPy 数组
    image_np = np.array(pil_image)

    # 确保图像是 RGB 格式，并转换为 (C, H, W) 格式
    if image_np.ndim == 2:  # 灰度图
        image_np = np.stack([image_np] * 3, axis=-1)  # 转换为 RGB
    image_np = image_np[:, :, :3]  # 只保留 RGB 通道

    # 将 NumPy 数组转换为 PyTorch 张量
    image = torch.tensor(image_np).permute((2, 0, 1)).byte().float().cuda()
...
class AEModel:
    ...
    def encode_frame(self, frame, isIframe = False, no_index_referesh=False):
        ...
        if isIframe:
            ...
            # encode I part
            # st = time.perf_counter()
            w_tot = w / self.w_step  # total number of parts in width
            h_tot = h / self.h_step  # total number of parts in height
            w_offset = int((self.p_index % w_tot) * self.w_step)  # offset in width
            h_offset = int(((self.p_index // w_tot) % h_tot) * self.h_step)  # offset in height
            #print(f"P_index = {self.p_index}, w_offset = {w_offset}, h_offset = {h_offset}")
            part_iframe = frame[:, h_offset:h_offset+self.h_step, w_offset:w_offset+self.w_step]  # part I frame
            icode, shapex, shapey, isize = bpg_encode(part_iframe)  # icode of I part
            # ed = time.perf_counter()
...
def encode_frame(ae_model: AEModel, is_iframe, ref_frame, new_frame, no_index_referesh=False):
    """
    encode a frame with the given ae_model
    
    Input:
        ae_model: the ae model
        is_iframe: True if the frame is I frame, False if the frame is P frame
...
def encode_whole_video(frames, ae_model: AEModel):
    ...
    """
    orig_frames = list(map(to_tensor, frames))  # to tensor
    codes = []
    dec_frames = []
    ref_frame = None
...
def run_one_model(model_id, input_pil_frames):
    ...
    dfs += [run_multi_frame_losses(1, TOTAL_FRAMES)]
    # dfs += [run_multi_frame_losses(3, TOTAL_FRAMES)]
    # dfs += [run_multi_frame_losses(5, TOTAL_FRAMES)]
    final_df = pd.concat(dfs)
    return final_df

def run_one_video(video):
    input_frames = read_video_into_frames(video, nframes=TOTAL_FRAMES)
